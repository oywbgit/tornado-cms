...
class Application(httputil.HTTPServerConnectionDelegate):
    ...
    你可以提供静态件服务通过传递 ``static_path`` 配置作为关键字
    参数. 我们将提供这些件从 ``/static/`` URI (这是可配置的通过
...
class StaticFileHandler(RequestHandler):
    ...
    为了自动的提供一个件例如 ``index.html`` 当一个目录被请求的时候,
    ...
    的缓存该件. `make_static_url` (也可作为 `RequestHandler.static_url`)
    可以被用来构造一个版本化的url.

    该处理程序主要用户开发和轻量级处理件服务; 对重型传输,使用专用的
    静态件服务是更高效的(例如nginx或Apache). 我们支持HTTP
    ...
    **子类注意事项**
    ...
    为了代替所有与件系统的相互作用(e.g. 从数据库提供静态内容服务),
...
class TemplateModule(UIModule):
    ...
    注意这些资源只在每个模板件中输出一次, 不是每次模板实例化一次,
    所以它们不能依赖于模板的任何参数.
    """
    def __init__(self, handler):
